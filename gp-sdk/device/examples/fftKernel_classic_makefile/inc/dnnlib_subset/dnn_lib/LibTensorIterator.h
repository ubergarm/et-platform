...
template <typename ElemTy> class HandleIterator {
public:
  // type definitions
  ...
  reference operator[](difference_type n) {
    return ptr_[offset_ + n];
  }
  reference operator*() {
    return ptr_[offset_];
  }

  bool operator==(const HandleIterator& x) {
  ...
  difference_type operator-(const HandleIterator& x) {
    return offset_ - x.offset_;
  }
  HandleIterator operator+(difference_type x) {
  ...
  const dim_array_t& coords() const {
    return coords_;
  }
