...
inline __attribute__((always_inline)) void loadFp32FromMemory(float* addr, float& res) {
#if 0
...
inline __attribute__((always_inline)) void storeFp32ToMemory(float* addr, float val32) {
#if 0
...
inline __attribute__((always_inline)) void storeFp16ToMemory(uint16_t* addr, float val32) {
#if 0
  // code for simd
  uint64_t tmp;
  __asm__ __volatile__("mov.m.x m0, zero, 0x1 \n"
...
inline __attribute__((always_inline)) float loadAndConvertToFp32(float* loadAddr) {
  float val16, val32;
...
inline __attribute__((always_inline)) void storeFp32(float* storeAddr, float val32) {
  storeFp32ToMemory(storeAddr, val32);
}

inline __attribute__((always_inline)) float loadAndConvertToFp16(float* loadAddr) {
  float val16, val32;
  loadFp32FromMemory(loadAddr, val16);
...
inline __attribute__((always_inline)) void storeFp16(uint16_t* storeAddr, float val32) {
...
template <class SrcTy, class DestTy> inline __attribute__((always_inline)) DestTy clip(SrcTy in) {
...
inline __attribute__((always_inline)) DestTy quantize(float input, float scale, int32_t offset) {
  float invertedScale;
...
inline __attribute__((always_inline)) float dequantize(SrcTy input, float scale, int32_t offset) {
...
template <class eTy>
inline __attribute__((always_inline)) float dequantizeWithFloatOffset(eTy input, float scale, float offset) {
...
inline __attribute__((always_inline)) int8_t quantizeValInt8(float val, float scale, int32_t offset) {
...
inline __attribute__((always_inline)) void loopAxis(Handle<ElemTy> srcH, Handle<ElemTy> destH,
  ...
  dim_array_t indicesDest = {
    0,
  };
  dim_array_t indicesSrc = {
    0,
  };
  dim_t ndx[max_tensor_dimensions] = {
    0,
  };
...
using UintSelector = typename std::conditional<
  T_numBytes == 1, uint8_t,
  typename std::conditional<T_numBytes == 2, uint16_t,
                            typename std::conditional<T_numBytes == 4, uint32_t, std::uint64_t>::type>::type>::type;
