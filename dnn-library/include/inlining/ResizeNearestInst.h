...
fwdLibResizeNearestInst(LibTensor* outT, LibTensor* inT, const std::array<float, N>& rszScale, uint64_t flags,
  ...
  for (size_t ob = 0; ob < outT->dims()[0]; ++ob) {
    ...
    /**/
    uint32_t x = static_cast<uint32_t>(((static_cast<uint32_t>(ob) * 1.0f) * invRszScale[0]));
    ...
    for (size_t oh = 0; oh < outT->dims()[1]; ++oh) {
      ...
      uint32_t y = static_cast<uint32_t>(((static_cast<uint32_t>(oh) * 1.0f) * invRszScale[1]));
      ...
      for (size_t ow = 0; ow < outT->dims()[2]; ++ow) {
        ...
        uint32_t t = static_cast<uint32_t>(((static_cast<uint32_t>(ow) * 1.0f) * invRszScale[2]));
        ...
        for (size_t oc = 0; oc < outT->dims()[3]; ++oc) {
          ...
          uint32_t z = static_cast<uint32_t>(((static_cast<uint32_t>(oc) * 1.0f) * invRszScale[3]));
